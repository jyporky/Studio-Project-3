...
public:
	...
	bool VolumeIncrease(float IncreaseAmount = 0.1f);
	// Decrease volume of a ISoundSource
	bool VolumeDecrease(float DecreaseAmount = 0.1f);

	void SetVolume(float volume);

	SOUNDFADE GetSoundFade(void);

	void SetSoundFade(SOUNDFADE newfade);

	MUSICTYPE GetMusicType(void);
	void SetMusicType(MUSICTYPE newMusicType);
