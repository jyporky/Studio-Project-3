...
bool CSoundController::LoadSound(	string filename,
									...
									CSoundInfo::MUSICTYPE eMusicType,
									const bool bPreload,
									...
									vec3df vec3dfSoundPos)
{
	...
	cSoundInfo->SetMusicType(eMusicType);
	//if (eMusicType != CSoundInfo::MUSICTYPE::NOT_MUSIC)
	//{
	//	cSoundInfo->VolumeDecrease(1);
	//}
...
void CSoundController::PlaySoundByID(const int ID)
{
	...
	if (pSoundInfo->GetMusicType() == CSoundInfo::NOT_MUSIC)
	{
		pSoundInfo->SetVolume(SFXVolume);
	}
...
void CSoundController::SetMasterVolume(double volume)
{
...
double CSoundController::GetMasterVolume(void)
{
	return cSoundEngine->getSoundVolume();
}

void CSoundController::SetMusicVolume(double volume)
{
	MusicVolume = volume;
	if (MusicVolume > 1)
		MusicVolume = 1;
...
double CSoundController::GetMusicVolume(void)
{
	return MusicVolume;
}

void CSoundController::SetSFXVolume(double volume)
{
	SFXVolume = volume;
	if (SFXVolume > 1)
		SFXVolume = 1;
...
double CSoundController::GetSFXVolume(void)
{
	return SFXVolume;
}
...
void CSoundController::UpdateSounds(double dt)
{
	//deal with fade in fade outs
	for (unsigned idx = 1; idx <= GetNumOfSounds(); idx++)
	{
		CSoundInfo* Sound = GetSound(idx);
		if (Sound == nullptr)
			continue;
		switch (Sound->GetSoundFade())
		{
		case CSoundInfo::SOUNDFADE::FADE_OUT:
			Sound->VolumeDecrease(Sound->FadeSpeed * dt);
			break;
		case CSoundInfo::SOUNDFADE::FADE_IN:
			Sound->VolumeIncrease(Sound->FadeSpeed * dt);
			break;
		}
	}
}

void CSoundController::SoundFadeout(const int ID, double time2fade)
{
	CSoundInfo* Sound = GetSound(ID);
	Sound->SetSoundFade(CSoundInfo::SOUNDFADE::FADE_OUT);
	Sound->FadeSpeed = 1 / time2fade;
}

void CSoundController::SoundFadeIn(const int ID, double time2fade)
{
	CSoundInfo* Sound = GetSound(ID);
	Sound->SetSoundFade(CSoundInfo::SOUNDFADE::FADE_IN);
	Sound->FadeSpeed = 1 / time2fade;
}

void CSoundController::ChangeMusic(unsigned ID)
{
	if (CurrMusic == ID)
		return;
